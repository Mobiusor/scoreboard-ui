...
class Rank extends React.Component {
  ...
  renderItems() {
    return this.state.rank.map((x, index) => {
	  ...
	  const ratingColorList = ['gray', 'green', 'cyan', 'blue', 'violet', 'orange', 'orange', 'red', 'red', 'black']
      ...
      const badgeColor = badgeColorMapping[index] ?? 'red'
      ...
      return (
        <div style={{margin: '8px', padding: '8px', borderBottom: '1px solid #ccc'}}>
          <span style={{marginRight: '32px'}}>
            <Badge count={index + 1} color={badgeColor}>
              <Avatar src={this.state.userMap[x.userId].avatar} />
            </Badge>
          </span>
          <span style={{ display: 'inline-block', width: '240px', textAlign:'left', marginRight: '32px', color: ratingColorList[ratingIndex] }} > {this.state.userMap[x.userId].name} ({ratingTitleList[ratingIndex] })</span>
